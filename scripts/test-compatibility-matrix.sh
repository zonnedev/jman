...
maven_bin="${JMAN_JAVA_LSP_MATRIX_MAVEN:-${tools_dir}/apache-maven-3.9.9/bin/mvn}"
...
gradle_bins=(
  "${JMAN_JAVA_LSP_MATRIX_GRADLE_8_7:-${tools_dir}/gradle-8.7/bin/gradle}"
  "${JMAN_JAVA_LSP_MATRIX_GRADLE_8_14_1:-${tools_dir}/gradle-8.14.1/bin/gradle}"
  "${JMAN_JAVA_LSP_MATRIX_GRADLE_9_1_0:-${tools_dir}/gradle-9.1.0/bin/gradle}"
)
...
for java_home in "${java_homes[@]}"; do
  ...
  JMAN_JAVA_LSP_MAVEN="${maven_bin}" \
  JMAN_JAVA_LSP_BUILD_JAVA_HOME="${java_home}" \
  JMAN_JAVA_LSP_MAVEN_REPOSITORY="${project_dir}/target/maven-repository" \
...
for gradle_bin in "${gradle_bins[@]}"; do
  if [[ ! -x "${gradle_bin}" ]]; then
    echo "Matrix Gradle is unavailable: ${gradle_bin} (set JMAN_JAVA_LSP_MATRIX_GRADLE_*)" >&2
    exit 1
  fi
  gradle_version="$(
  ...
  for java_home in "${java_homes[@]}"; do
    ...
    JMAN_JAVA_LSP_GRADLE="${gradle_bin}" \
    JMAN_JAVA_LSP_BUILD_JAVA_HOME="${java_home}" \
